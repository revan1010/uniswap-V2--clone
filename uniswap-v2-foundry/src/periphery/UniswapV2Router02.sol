        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            { // scope to avoid stack too deep errors
                ...
                { // scope to avoid stack too deep errors
                    ...
                    if (input == token0) {
                        amountInput = IERC20(input).balanceOf(address(pair)) - balance0;
                        amountOutput = UniswapV2Library.getAmountOut(amountInput, balance0, balance1);
                    } else {
                        amountInput = IERC20(input).balanceOf(address(pair)) - balance1;
                        amountOutput = UniswapV2Library.getAmountOut(amountInput, balance1, balance0);
                    }
                }
            }
